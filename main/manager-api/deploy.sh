...
APP_BASE="/home/deployer/projects/running-apps/xiaozhi-api"
...
# 启动应用，日志写到logs目录
mkdir -p "$LOG_DIR"
nohup /home/deployer/.sdkman/candidates/java/current/bin/java  $JAVA_OPTS -jar "$JAR_NAME" --spring.profiles.active=prod > "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"
